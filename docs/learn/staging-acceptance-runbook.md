...
> 🟡 **2026-09-06 實況：staging 與正式環境都沒有任何考卷**，
> 所以步驟 2–4 做不了，`exam_attempts` 也不會有列。
> **上線時學生看到的就是步驟 1 的空狀態**，這次驗收到此為止是完整的。
>
> 但要記錄：有資料時的顯示路徑**在任何環境都尚未驗證**。
> 另外 `exam_attempts.total_score` 沒有任何地方會寫入
> （交卷只寫 `submitted_at` / `time_spent_seconds` / `status`），
> 所以即使將來有人交卷，分數欄很可能顯示 `—`。
> 兩者都記在 `docs/learn/security-followups.md` §7。
