...
## 🟡 7. 「最近的成績」有資料時的路徑從未被驗證過

staging 與正式環境**都沒有任何考卷**，所以 `exam_attempts` 不會有列，
Dashboard 的「最近的成績」永遠顯示空狀態。**上線時學生看到的就是這個**，
所以這次驗收是完整的 —— 但有兩件事留著：

**① 有資料時的顯示路徑是潛在未驗程式碼。**
建第一份考卷、有人交卷之後，要回頭確認：考卷名稱、日期、分數顯示正確，
而且**沒有**出現任何由題型分數換算的能力值。

**② `exam_attempts.total_score` 沒有任何地方會寫入。**
`submitAttempt()` 只寫 `submitted_at` / `time_spent_seconds` / `status`；
程式裡另外兩處寫 `total_score` 的是 **`exams` 表**（考卷的總分設定），
不是作答紀錄。稽核也記過 exam 領域有未使用的批改欄位
（`graded_by` / `graded_at` / `grader_feedback`）。

除非資料庫有觸發器代算，否則分數欄會顯示 `—`。
**這是 `/exam` 保留領域既有的缺口，不是 Dashboard 的問題**，
修它要動保留領域，不在這次範圍內。
