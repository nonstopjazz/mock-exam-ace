...
-- 🛑 這個方法有一個盲點，判讀時務必記得：
--
--    【零錯誤的作文驗不出 A9】。它不產生任何 findings 列，就沒有 created_at
--    可以比對，所以永遠落在「⚪ 零錯誤」那一類 —— 不管是 A9 同步的還是回填的。
--
--    也就是說：A9 部署之後，若下一篇作文剛好【沒有任何錯誤】，
--    這張表不會出現 ⏱ 秒級那一列。那【不代表 A9 壞掉】。
--
--    要確認 A9 對零錯誤作文也有跑，只能看 Vercel log ——
--    成功時它【不印任何東西】（這是刻意的，見 verify 腳本的 E3），
--    所以沒有「findings 同步失敗」就是正常。
--
--    ➡️ 結論：要驗 A9，等一篇【有錯誤】的作文。那才會出現 ⏱ 秒級。
-- =====================================================
WITH latest AS (
