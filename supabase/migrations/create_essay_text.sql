-- =====================================================
-- Migration: 建立 essay_text 表（寫作系統 Phase 1）
--
-- essay_text 是「正規作文文字」—— 未來 AI evidence 的字元位移都以它為基準。
--
-- 兩條合法的輸入路徑（rev 4 決策 L9）：
--   文字提交 ─────────────────────────▸ essay_text  provenance = TYPED
--   圖片提交 ─▸ assets ─▸ ocr_run ────▸ essay_text  provenance = OCR
...
CREATE TABLE IF NOT EXISTS essay_text (
...
-- 推導「目前文字」用
CREATE INDEX IF NOT EXISTS idx_essay_text_current
  ON essay_text (essay_id, created_at DESC);

COMMENT ON TABLE essay_text IS
  '正規作文文字，append-only。AI evidence 的字元位移以此為基準，因此永不就地修改；修正一律插入新列。';
COMMENT ON COLUMN essay_text.provenance IS
  'TYPED = 學生直接打字；OCR = 由 OCR run 產生（Phase 2）；OCR_CORRECTED = 人工修正過的辨識結果（Phase 4）。';
COMMENT ON COLUMN essay_text.content IS
  '未經 trim 或正規化的原始文字。任何清理都會讓字元位移與學生實際寫下的內容產生偏移。';

-- =====================================================
-- Append-only：禁止 UPDATE
--
-- 只擋 UPDATE，不擋 DELETE。原因：essay_submissions 的 ON DELETE CASCADE
-- 會觸發本表的 row trigger，若這裡擋 DELETE，連帶刪除會直接失敗，
-- 學生也就永遠刪不掉自己的作文。直接刪除改由 RLS 擋 —— 本表沒有 DELETE 政策，
-- 而 cascade 是系統操作，不受 RLS 限制。
-- =====================================================

CREATE OR REPLACE FUNCTION essay_text_guard_append_only()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
BEGIN
  RAISE EXCEPTION 'essay_text 為 append-only，不可修改（id=%）。修正請插入新的一列。', OLD.id;
END;
$$;

DROP TRIGGER IF EXISTS trg_essay_text_guard_append_only ON essay_text;
CREATE TRIGGER trg_essay_text_guard_append_only
  BEFORE UPDATE ON essay_text
  FOR EACH ROW
  EXECUTE FUNCTION essay_text_guard_append_only();

-- =====================================================
-- RLS
-- =====================================================

ALTER TABLE essay_text ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Students can view own essay text" ON essay_text;
CREATE POLICY "Students can view own essay text"
  ON essay_text FOR SELECT
  USING (EXISTS (
    SELECT 1 FROM essay_submissions e
    WHERE e.id = essay_text.essay_id AND e.student_id = auth.uid()
  ));

DROP POLICY IF EXISTS "Admins can view all essay text" ON essay_text;
CREATE POLICY "Admins can view all essay text"
  ON essay_text FOR SELECT
  USING (is_admin());

-- 只能在自己的、尚未送出的作文上寫入文字
DROP POLICY IF EXISTS "Students can insert own essay text" ON essay_text;
CREATE POLICY "Students can insert own essay text"
  ON essay_text FOR INSERT
  WITH CHECK (EXISTS (
    SELECT 1 FROM essay_submissions e
    WHERE e.id = essay_text.essay_id
...
-- =====================================================
-- submit_text_essay() —— 一次交易完成整個送出流程
--
...
CREATE OR REPLACE FUNCTION submit_text_essay(
  p_title         TEXT,
  p_content       TEXT,
...
BEGIN
  ...
  -- content 刻意不 trim
  INSERT INTO essay_text (essay_id, content, provenance, created_by)
...
COMMENT ON FUNCTION submit_text_essay IS
  '在單一交易中建立文字作文：草稿 → 正規文字 → 送出。SECURITY INVOKER，RLS 全程生效。';

GRANT EXECUTE ON FUNCTION submit_text_essay(TEXT, TEXT, TEXT, DATE, TEXT) TO authenticated;
