    ...
    <WorkspaceSection
      ...
      action={
        <>
          ...
          {isGroup && (
            <Button variant="outline" size="sm" className="h-8" onClick={markAllHomeworkDone}>
    ...
    >
      <div className="divide-y divide-border/60">
        {scenario.students.map((s) => {
          ...
          return (
            <div key={s.id} className="flex flex-wrap items-center gap-2.5 py-1.5">
              ...
              {!entry && <span className="text-xs text-muted-foreground">尚未記錄</span>}
            </div>
          );
        })}
      </div>
